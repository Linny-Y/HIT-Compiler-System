...
class IRSim(QMainWindow, ui_mainwindow.Ui_MainWindow):
    ...
    def __init__(self, parent=None):
        ...
        # self.connect(self.actionStep.triggered.connect(self.step)
    ...
    def getValue(self, var):
        if var[0] == '#':
            ...
            if var[0] == '*':
                return self.mem[int(self.mem[int(self.symTable[var[1:]][0] / 4)] / 4)]
            return self.mem[int(self.symTable[var][0] / 4)]
    ...
    #            strs = str(self.mem[int(self.symTable[key][0] / 4):int(self.symTable[key][0] / 4 + self.symTable[key][1] / 4)])
    #            item = QTableWidgetItem(strs)
    #        else:
    #            item = QTableWidgetItem(str(self.mem[int(self.symTable[key][0] / 4)]))
    #        item.setTextAlignment(Qt.AlignRight | Qt.AlignVCenter)
    ...
    def execute_code(self, code):
        ...
        try:
            if code[0] == 'READ':
                ...
                if ok:
                    self.mem[int(self.symTable[code[1]][0] / 4)] = result
            elif code[0] == 'WRITE':
                ...
                if code[1][0] == '*':
                    self.mem[int(self.mem[int(self.symTable[code[1][1:]][0] / 4)] / 4)] = value
                else:
                    self.mem[int(self.symTable[code[1]][0] / 4)] = value
            elif code[0] == 'ARITH':
                value1 = self.getValue(code[2])
                value2 = self.getValue(code[4])
                self.mem[int(self.symTable[code[1]][0] / 4)] = eval(str(value1) + code[3] + str(value2))
            elif code[0] == 'RETURN':
                ...
                self.mem[int(self.symTable[stackItem[1]][0] / 4)] = returnValue
                ...
                self.mem[int(self.symTable[code[1]][0] / 4)] = self.argumentStack.pop()
        except IndexError:
            return 2
