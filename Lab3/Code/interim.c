...
void translate_FunDec(P_Node now){
    ...
    func_op->u_char=now->firstchild->char_name;
    ...
    //翻译varlist，参数列表
    if(now->firstchild->nextbro->nextbro->nextbro==NULL) return;
    Function find_func=find_in_function(now->firstchild->char_name);
    ...
    while(func_field!=NULL){
        ...
        struct TABLE* find_it=find_struct_in_table(func_field->Token);
        field_op->u_int=find_it->variable;
        field_op->u_char=func_field->Token;
        //2.每个参数生成一条中间代码
...
//printf("a translate_DecList\n");
    ...
    if(comma==NULL);
    else translate_DecList(comma->nextbro);
}
...
void translate_Exp(P_Node now,Operand place){
    ...
    if(strcmp(fir_bro->Token,"ASSIGNOP")==0){
        ...
        if(strcmp(child->firstchild->Token,"ID")==0){
            ...
            Operand t2=(Operand)malloc(sizeof(struct Operand_));
            struct TABLE* find_it=find_struct_in_table(child->firstchild->char_name);
            t2->kind=VARIABLE;
            t2->u_char=child->firstchild->char_name;
    ...
    if(strcmp(fir_bro->Token,"DOT")==0){
        ...
        if(t1->kind==VARIABLE){
            ...
            struct TABLE* F=find_struct_in_table(fir_bro->nextbro->char_name);
    ...
    if(strcmp(fir_bro->Token,"LB")==0){
        ...
        if(t1->kind==VARIABLE){
            ...
            //printf("a %s %d\n",child->firstchild->char_name,offset);
            if(offset==-1) return;
            //printf("a offset:%d\n",offset);
            //3.生成t2=offset*i
            Operand i=new_temp();
...
void translate_Cond(P_Node now,Operand lt,Operand lf){
    ...
    if(child->nextbro!=NULL&&strcmp(child->nextbro->Token,"RELOP")==0){
        ...
        op->kind=RELOP;
        op->u_int=child->nextbro->linenumber;
        //code3
...
void translate_VarDec(P_Node now,Operand place){
    ...
    if(now->firstchild->nextbro==NULL){
        ...
        struct TABLE* find_it=find_struct_in_table(now->firstchild->char_name);
        //printf("a is_find?%d\n",find_it);
        FieldList find_field=find_it->field;
        if(find_field->type->kind==BASIC){
            //printf("a isBASIC\n");
            //printf("a %s\n",now->firstchild->char_name);
            if(place==NULL) return;
            place->kind=VARIABLE;
            place->u_int=find_it->variable;
            place->u_char=now->firstchild->char_name;
            return;
        }
        ...
        place->kind=VARIABLE;
        place->u_char=now->firstchild->char_name;
        struct TABLE* find_it=find_struct_in_table(now->firstchild->firstchild->char_name);
        place->u_int=find_it->variable;
        //生成DEC语句
...
//Args → Exp COMMA Args| Exp
    ...
    exp=exp->nextbro;
    if(exp==NULL);
    else translate_Args(exp->nextbro,code1_in);
    if(here==NULL){
...
void translate_StructSpecifier(P_Node now){
    ...
    if(sec_bro==NULL){
        //Tag → ID,生成DEC语句
        Operand t1=(Operand)malloc(sizeof(struct Operand_));
        struct TABLE* find=find_struct_in_table(fir_bro->firstchild->char_name);
        t1->kind=VARIABLE;
        t1->u_int=find->variable;
        t1->u_char=fir_bro->firstchild->char_name;
...
int get_offset(Type return_type,P_Node after){
    ...
    if(return_type->kind==BASIC) return 4;
        ...
        while(count!=NULL){
            if(strcmp(count->Token,after->char_name)==0) break;
...
//void translate_ExtDecList(P_Node now,Type type);
//FieldList translate_ParamDec(P_Node now,int judge);
